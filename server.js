...
app.get('/api/health', (_req,res)=>sendJson(res,200,{ok:true,service:'anant-vijay',version:'3.0.0'}));
...
app.post('/api/upload-poster',adminOnly,(req,res)=>{
  upload.single('poster')(req,res,(err)=>{
    ...
    if(!req.file) return sendJson(res,400,{ok:false,error:'Please select a JPG/JPEG file.'});
    ...
    try {
      ...
      if(!isJpeg) {
        ...
        return sendJson(res,400,{ok:false,error:'The selected file is not a valid JPG/JPEG image.'});
      }
    } catch {
      return sendJson(res,500,{ok:false,error:'Could not verify the uploaded JPG file.'});
    }

    sendJson(res,200,{ok:true,url:`/uploads/${encodeURIComponent(req.file.filename)}`,name:req.file.originalname});
  });
});
