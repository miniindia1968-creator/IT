...
app.post('/api/upload-poster',adminOnly,(req,res)=>{
  upload.single('poster')(req,res,(err)=>{
    ...
    if(!req.file){
      return sendJson(res,400,{
        ok:false,
        error:'No JPG file was received by the server. Please choose the JPG again and retry.'
      });
    }
    ...
    try {
      ...
      if(!isJpeg){
        ...
        return sendJson(res,400,{
          ok:false,
          error:'The selected file is not a valid JPG/JPEG image. Please convert it to JPG and try again.'
        });
      }
    } catch(err){
