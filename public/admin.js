...
async function uploadPoster(i, input){
  ...
  if(!file) return;
  if(!/\.jpe?g$/i.test(file.name) && file.type!=="image/jpeg"){alert("Please select a JPG/JPEG file."); input.value=""; return;}
  if(file.size>5*1024*1024){alert("JPG must be 5 MB or smaller."); input.value=""; return;}
  const status=document.getElementById("uploadStatus"+i);
  if(status) status.textContent="Uploading...";
  try{
    const fd=new FormData(); fd.append("poster",file);
    const r=await fetch("/api/upload-poster",{method:"POST",headers:{Authorization:"Bearer "+token},body:fd});
    const x=await r.json();
    if(!r.ok) throw new Error(x.error||"Upload failed");
    data.movies[i].poster=x.url;
    if(status) status.textContent="JPG uploaded successfully";
    render();
  }catch(e){alert(e.message||"Poster upload failed."); if(status) status.textContent="Upload failed";}
}
