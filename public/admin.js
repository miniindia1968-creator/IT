...
function movieEditor(m,i){
  return `<div class="movie">
    ...
    <div class="poster-tools">
      ...
      ${m.poster?`<img class="preview" style="display:block" src="${esc(m.poster)}?v=${Date.now()}" alt="Poster preview" loading="lazy">`:''}
    </div>
...
async function uploadPoster(i,input){
  ...
  try{
    // Send the image as JSON/base64. This deliberately avoids multipart/form-data
    // because some Render/browser/proxy combinations were reaching the server
    // without the multipart file part even though the browser showed the file.
    const dataUrl=await new Promise((resolve,reject)=>{
      const reader=new FileReader();
      reader.onload=()=>resolve(String(reader.result||''));
      reader.onerror=()=>reject(new Error('Could not read the selected JPG.'));
